...
import pandas as pd
import numpy as np
pd.options.mode.chained_assignment = None  # default='warn', suppress warning

# attempts
# 253907829, correct answer
...
def detect_repeated_characters(string):
    ...
    for char in string:
        if char == "J":
            ...
            chars[char] = 1
    return np.append(sorted(chars.values(), reverse=True), numjacks)
    ...
    for i in df_array['Hand'].index:
        for j in df.loc[i, 'Hand']:
            match j:
                ...
                case "J":
                    df_array.loc[i, 'Converted'] = df_array.loc[i, 'Converted'] + "1"
                case "Q":
...
def apply_jacks(int_array):
    num_jacks = int_array[-1]
    if num_jacks < 5:
        int_array = np.delete(int_array, -1)
        if num_jacks > 0:
            int_array[0] += num_jacks
    return int_array


# Read in data, save to dataframe
read_data = []
file = open('C:/Users/andjo/Repo/AdventOfCode2023/src/Day07/input07.txt', 'r')
for i in file:
    parts = i.strip().split(" ")
